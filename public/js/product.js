...
$(document).ready(function() {
	...
	manageProductTable = $('#manageProductTable').DataTable({
		'ajax': `${API_URL}/sys/maintenance/list`,
		'order': []
	});
	...
	$("#addProductModalBtn").unbind('click').bind('click', function() {
		...
		$("#productImage").fileinput({
		    ...
		    defaultPreviewContent: `<img src="${API_URL}/public/images/photo_default.png" alt="Profile Image" style="width:100%;">`,
		    layoutTemplates: {main2: '{preview} {remove} {browse}'},								    
	  		allowedFileExtensions: ["jpg", "png", "gif", "JPG", "PNG", "GIF"]
		});   
		// submit product form
		$('#submitProductForm').submit(async function(e){
			e.preventDefault();
			$('.text-danger').remove();
				...
				if(response.status == 200 && response.data.status == true){
					...
					$('#add-product-messages').html(`
						<div class="alert alert-success">
							...
							<strong><i class="glyphicon glyphicon-ok-sign"></i></strong> ${response.data.message}
						</div>
					`);
					// remove the mesages
